...
**Recommended order once approved:** C2 (2 CNN seeds — protects a headline) → **Phase 3 autoencoder (~1h — tests the reframing)** → then re-decide whether the KG is the right next build → C1/C3 reporting variants (no training) → C4 → C5. **LOCO/fusion-repair work is deprioritized** — it attempts to repair an (A)-family method, and the BeaconLike rotation was refuted in advance (fires on 97.6% of PortScan, 0.0% of every other known attack).
