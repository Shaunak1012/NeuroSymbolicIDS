...
✅ **Phase 3 (benign-only autoencoder) is BUILT AND RUN (2026-08-02).** Read **"🧪 PHASE 3 RESULTS"** in STATUS — it changed the architecture story. Short version: the AE gets the 2nd-best Bot number ever measured (3.6×) and catches Heartbleed at 1.0000 recall / Infiltration at 0.8611, but scores **exactly 0.0000 recall on every web attack** (macro 0.1000 vs the CNN's 0.6399). That categorical split refined the thesis: **the governing variable is not (A) vs (B) method family, but whether the unseen class shares a behavioural modality with some known class.** Web attacks resemble the Patator classes that *are* in training (so the CNN transfers); Bot/Infiltration/Heartbleed have no known-class analogue (so only distance/reconstruction methods work). **The families are complementary and the system needs a per-flow router** — which is exactly what the fusion wall stops you from fitting, and the job the KG could plausibly do without labels. **n=1, and the modality account is an interpretation, not yet measured.**

**Recommended order:** ~~C2~~ ✅ done → ~~Phase 3 autoencoder~~ ✅ done → **measure the modality account** (embedding distance from each zero-day family to nearest known-class centroid — converts the story from narrative to falsifiable law) and/or **multi-seed the AE** → then decide the KG's scope, now that it has a well-motivated job (routing, not detection) → C1/C3 reporting variants (no training) → C4 → C5. **LOCO/fusion-repair remains deprioritized**; the BeaconLike rotation was refuted in advance (fires on 97.6% of PortScan, 0.0% of every other known attack) — see STATUS "THE FUSION WALL" for the revised Mahalanobis-based version if that line is picked back up.
