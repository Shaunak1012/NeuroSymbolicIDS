...
Short version of what is actually established: the AE is a **raw-space distance-from-benign detector** (confirmed, `corr = +0.732`), it is the best Bot channel after Mahalanobis (**3.6× vs 4.3×**) — but the CNN is 1.7×, so that comparison is **two weak methods, not a win** — and it is excellent on Heartbleed (103×) / Infiltration (145×), which are the **underpowered** families `metrics.py` excludes. **On lift the AE is comparably weak across all powered families (Bot 3.6×, Web BF 4.4×, XSS 5.3×)** — the dramatic "0.0000 recall on web attacks" split I first reported was a **threshold artifact**, not a real categorical difference.
...
**Recommended order:** ~~C2~~ ✅ → ~~Phase 3 autoencoder~~ ✅ → ~~measure the modality account~~ ✅ (falsified it) → **multi-seed the AE** (n=1 today; C2 showed seed noise can swallow gaps this size) and treat **"why is the CNN specifically so bad on Bot"** as the live open question (the oracle result proves the information *is* in the features, so it is a transfer failure with no established mechanism) → then re-decide the KG's scope → C1/C3 reporting variants (no training) → C4 → C5. **LOCO/fusion-repair stays deprioritized** — and note the per-flow "router" idea rested on the now-falsified mechanism, so it is no longer motivated as-is.
