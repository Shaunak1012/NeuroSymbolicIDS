...
**Recommended order:** ~~C2~~ ✅ → ~~Phase 3 AE~~ ✅ → ~~modality test~~ ✅ (falsified) → ~~multi-seed the AE~~ ✅ (established the dissociation) → **next, in value order: (a) why is the CNN specifically so bad on Bot** — the oracle result (0.0314 → 0.9764 with ~1,000 labels) proves the information *is* in the features, so it's an unexplained transfer failure; **(b)** whether the dissociation is exploitable at all given the fusion wall; **(c)** multi-seed Mahalanobis (n=1, Bot 0.1467) to upgrade "(B)-family" from 2 points to a real family claim → then re-decide the KG's scope → C1/C3 reporting variants (no training) → C4 → C5. **LOCO/fusion-repair stays deprioritized**, and the per-flow "router" idea rested on the falsified mechanism — not motivated as-is.
