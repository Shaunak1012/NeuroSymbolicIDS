...
| **The base paper's own metric set** | We reproduce their 1D CNN's zero-day number almost exactly (**47.85 % vs 48.34 %**) and **cannot reproduce the Hybrid-LTN's +12 pp gain** (~~`ltn_repro` 47.24 %~~ matched control 2026-09-16: SAT term **+0.55 pp**, not direction-consistent) | ⚠️ Comparison in **form, not head-to-head**: different modality (flow features vs payload bytes), zero-day membership differs by a swap (they hold out PortScan, train Infiltration; we do the reverse), class sizes differ. **Composition explains only ~4 pp of the missing 12 pp** — so it is not explained away, but say what is controlled. |
...
| **The symbolic pillar itself** | **−0.0004 (n.s.)** alone; ~~**significantly HURTS**~~ *(withdrawn 2026-09-17 — three-run artefact; the matched axiom-vs-control comparison is negative with all 17 CNN runs)* on top of the KG (0.6926 → 0.6708, **p<0.0001**), diluting Bot 0.2518 → 0.2043 | 🔑 This is a **negative result about our own architecture** — lead with it rather than burying it. |
| **A fitted fuser over the channel that actually helps** | **Structurally impossible, and this is the surviving form of the fusion wall.** `kg.py`'s burstiness is defined by streaming the **test** set into windows, so the KG has **no validation-side score at all** — the channel with the largest measured gain (**+0.0527, 3/3**) cannot enter a fitted combiner under any protocol that fits on held-out data. | 🔴 **State the scope, because we got it wrong once.** The wall applies to channels whose value is **zero-day-specific**. It does **not** apply to a channel that also helps on known classes — see §6, where a fitted combiner over CNN + autoencoder does work. `fitted_fusion.py`. |
...
| claim | evidence | caveat |
|---|---|---|
| **A fitted fuser DOES work over CNN + autoencoder** — macro **0.6502 vs the CNN's 0.6399** (**+0.0103, 3/3 seeds**), and **+0.0604 over equal-weight rank fusion of the same two channels** (3/3, **2.5σ**). It gives the autoencoder **17.9 % of absolute weight, positive every seed** | `fitted_fusion.py`, fitted on validation (**0 zero-day flows, asserted in code**), applied blind to test, achieved FPR 0.0100 exactly on all 3 seeds | ⚠️ **+0.0103 is 0.80σ — direction established (3/3), magnitude NOT.** Report it as *"a fitted combiner is possible and marginally positive"*, never as *"fitted fusion works"*. It does **not** replace CNN+KG (+0.0527, magnitude 0.027–0.088). 🔴 **This row is a RETRACTION of our own §5 claim** that a fitted fuser is structurally impossible — that claim was generalised from `fusion_beaconlike.py`, a two-channel special case whose `[2.35, 0.02]` was a property of **BeaconLike**, not of fitted fusion. |
...
- 🔴 **A NEGATIVE claim needs the same evidentiary standard as a positive one, and ours did not get it.** We wrote *"a fitted fuser is structurally impossible here"* on the strength of **one two-channel special case** (`fusion_beaconlike.py`, coefficients `[2.35, 0.02]`). Running it on the real channel set **falsified all three of its predictions** and the pre-registered falsifier fired: the combiner gives the anomaly channel 17.9 % of weight and beats parameter-free fusion by +0.0604 (3/3, 2.5σ). **Every safeguard in this project was pointed at over-claiming a positive result; this one was an over-claimed *blocker*, and nothing was watching that direction.** The surviving claim is narrower and still sufficient — the KG has no validation-side score by construction — but it had to be measured to be found.
...
| **The base paper: CNN reproduced, symbolic gain not** | Bizzarri et al. ICCCN 2024 · LTN (Badreddine et al.) · their 2025 survey | 47.85 % vs 48.34 %; ~~`ltn_repro` 47.24 %~~ matched control: SAT term +0.55 pp vs their +12.13. ⚠️ **Form, not head-to-head** (§3d). ⚠️ **Say our own pillar fails too** — otherwise it reads as an attack on theirs. |
