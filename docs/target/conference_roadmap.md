...
| **Architecture** *(added 2026-09-16, FD-03)* | Conv1D 32/64/128, MaxPool **1×4 / 1×8 / 1×16**, Dense(N×5) → Dense(N), no dropout, batch **128** | Conv1D 32/64/128, MaxPool **2 / 2 / 2**, Dense(64) → Dropout 0.4 → Dense(32) → Dropout 0.3 → Dense(N), L2 1e-4, batch **256** | Pooling is forced by input width (1500 vs 68); the head is ours |
