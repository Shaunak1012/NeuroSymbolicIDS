...
| **Balance, in full** *(added 2026-09-16, audit FD-01)* | **Every** known attack class equalised to 31,843; benign fixed at 200,000 | Benign 1 : 1 against the **total** known-attack count; attack classes keep natural proportions (41.8 : 1) | Not reproduced. Measured: balanced accuracy moves our known-class gap only +18.82 → +18.79 pp |
| **Unit** *(added 2026-09-16, FD-05)* | Payload-bearing **packet** | **Flow** | No Table I count is comparable to a flow count (their Heartbleed 13,486 packets = our 11 flows = one connection) |
| **Cleaning** *(added 2026-09-16, FD-02)* | Duplicates **and payload-less records deleted** | Neither deleted (17.02 % train/test duplicates) | Dedup moves our known-class accuracies ≤ 0.71 pp. The payload-less filter ≈ Engelen's `X - Attempted`, so the zero-day populations differ |
| **Architecture** *(added 2026-09-16, FD-03)* | Conv1D 32/64/128 (Fig. 2 prints conv2 as "63 filters", a typo — BP-07), MaxPool **1×4 / 1×8 / 1×16**, Dense(N×5) → Dense(N), no dropout, batch **128** | Conv1D 32/64/128, MaxPool **2 / 2 / 2**, Dense(64) → Dropout 0.4 → Dense(32) → Dropout 0.3 → Dense(N), L2 1e-4, batch **256** | Pooling is forced by input width (1500 vs 68); the head is ours |
